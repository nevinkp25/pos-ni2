    ...
    <div className="flex flex-col h-screen bg-[#f8fbfe] font-sans text-[#1a1c2e] safe-top safe-bottom overflow-hidden relative tracking-normal">
      ...
      <div className="flex-1 px-4 pt-6 overflow-y-auto pb-10 space-y-6">
        ...
        <div className="bg-white rounded-[32px] py-10 px-6 shadow-[0_15px_40px_rgba(0,0,0,0.03)] border border-gray-50 flex flex-col items-center justify-center text-center">
          ...
          <div className="flex items-center gap-2 text-[#0066b2] font-black text-[42px] leading-none"><CurrencySymbol className="text-[38px]" /><span>{shareAmount.toFixed(2)}</span></div>
        </div>
        ...
        {isConfirmed && (<div className="pt-2 animate-in fade-in slide-in-from-top-4 duration-500"><span className="text-[11px] font-black text-[#94a3b8] uppercase tracking-widest mb-4 block">Select who is paying</span><div className="space-y-3">{Array.from({ length: guestCount }).map((_, i) => { const guestId = i + 1; const isPaid = paidGuests.includes(guestId); const isOpen = expandedGuest === guestId; return (<Collapsible key={guestId} open={isOpen} onOpenChange={() => setExpandedGuest(isOpen ? null : guestId)} className={cn("bg-white rounded-[24px] shadow-[0_10px_30px_rgba(0,0,0,0.03)] border border-gray-50 overflow-hidden transition-all", isPaid && "opacity-80")}><div className="p-4 flex items-center justify-between"><div className="flex items-center gap-4"><div className="w-12 h-12 rounded-full bg-[#f8fbfe] flex items-center justify-center border border-gray-50"><User className="w-6 h-6 text-[#0066b2]" /></div><div className="flex flex-col"><div className="flex items-center gap-2"><span className="text-[15px] font-black text-[#1a1c2e]">Guest {guestId}</span><CollapsibleTrigger asChild><button className="text-gray-400">{isOpen ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}</button></CollapsibleTrigger></div><div className="flex items-center gap-1"><CurrencySymbol className="text-[#94a3b8] text-[10px]" /><span className="text-[11px] font-bold text-[#94a3b8]">{shareAmount.toFixed(2)}</span></div></div></div>{isPaid ? (<div className="h-10 px-4 rounded-full bg-[#ecf7ef] text-[#26ab5f] flex items-center gap-2 text-[12px] font-black uppercase"><Check className="w-3.5 h-3.5 stroke-[4px]" />Paid</div>) : (<Button onClick={() => handlePayClick(guestId)} className="h-10 px-6 rounded-full bg-[#0066b2] text-white text-[12px] font-black uppercase shadow-md active:scale-95">Pay</Button>)}</div><CollapsibleContent className="px-5 pb-5"><div className="bg-[#f8fbfe] rounded-[18px] p-4 space-y-3 border border-gray-50/50"><div className="flex justify-between items-center text-[12px] font-black text-[#94a3b8]"><span className="uppercase">Service Fee</span><div className="flex items-center gap-1"><CurrencySymbol className="text-[10px]" /><span>{(shareAmount * 0.1).toFixed(2)}</span></div></div><div className="flex justify-between items-center text-[12px] font-black text-[#94a3b8]"><span className="uppercase">VAT (5%)</span><div className="flex items-center gap-1"><CurrencySymbol className="text-[10px]" /><span>{(shareAmount * 0.05).toFixed(2)}</span></div></div><div className="pt-2 border-t border-dashed border-gray-200 flex justify-between items-center"><span className="text-[11px] font-black text-[#1a1c2e] uppercase">Total</span><div className="flex items-center gap-1.5 text-[#0066b2] font-black text-[16px]"><CurrencySymbol className="text-[14px]" /><span>{shareAmount.toFixed(2)}</span></div></div></div></CollapsibleContent></Collapsible>); })}</div></div>)}
      </div>
      ...
      <Sheet open={isSettlementOpen} onOpenChange={setIsSettlementOpen}>
        <SheetContent side="bottom" className="rounded-t-[32px] border-none p-0 outline-none overflow-hidden max-h-[92vh] flex flex-col tracking-normal">
          <SheetHeader className="sr-only"><SheetTitle>Final Review</SheetTitle></SheetHeader>
          <div className="flex-1 overflow-y-auto pb-10">
            ...
            <div className="px-6 flex items-center justify-between mb-6">
              ...
              <button onClick={() => setIsSettlementOpen(false)} className="w-9 h-9 flex items-center justify-center rounded-full bg-gray-50 text-gray-400 active:scale-90 transition-all"><X className="w-5 h-5" /></button>
            </div>
            ...
            <div className="px-6 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-white rounded-[24px] p-5 shadow-[0_10px_30px_rgba(0,0,0,0.03)] border border-gray-50 flex flex-col items-center justify-center min-h-[120px]">
                  <span className="text-[9px] font-black text-[#94a3b8] uppercase mb-2 tracking-tight">Bill Amount</span><div className="flex items-center gap-2"><CurrencySymbol className="text-[20px] text-[#0066b2]" /><span className="text-[24px] font-black text-[#0066b2] tabular-nums">{shareAmount.toFixed(2)}</span></div>
                </div>
              </div>
                ...
                <div className="grid grid-cols-4 gap-3">
                  {[5, 10, 20].map((amount) => (
                    <button key={amount} onClick={() => handleTipClick(amount)} className={cn("relative h-[90px] rounded-[24px] flex flex-col items-center justify-center transition-all shadow-[0_10px_30px_rgba(0,0,0,0.03)] border border-gray-50", (!isCustomTipMode && selectedTip === amount) ? "bg-[#f0f7ff] border-[#0066b2] border-2" : "bg-white")}>
                      <CurrencySymbol className="text-[10px] text-[#94a3b8] uppercase" /><span className="text-[22px] font-black text-[#1a1c2e] tabular-nums">{amount}</span>{(!isCustomTipMode && selectedTip === amount) && <div className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-[#ef4444] rounded-full flex items-center justify-center border-2 border-white"><X className="w-3 h-3 text-white stroke-[4px]" /></div>}
                    </button>
                  ))}
              ...
              <div className="bg-white rounded-[32px] p-6 shadow-[0_20px_60px_rgba(0,0,0,0.05)] border border-gray-50 space-y-4">
                <div className="flex justify-between items-center text-[15px] font-black"><span className="text-[#94a3b8] uppercase">Bill Amount</span><div className="flex items-center gap-1.5 text-[#1a1c2e]"><CurrencySymbol className="text-[15px]" /><span className="tabular-nums">{shareAmount.toFixed(2)}</span></div></div>
                <div className="flex justify-between items-center text-[15px] font-black"><span className="text-[#94a3b8] uppercase">Tips</span><div className="flex items-center gap-1.5 text-[#26ab5f]"><span>+</span><CurrencySymbol className="text-[15px]" /><span className="tabular-nums">{currentTipAmount.toFixed(2)}</span></div></div>
                <div className="w-full border-t border-dashed border-gray-100 py-1" />
                <div className="flex justify-between items-center"><span className="text-[13px] font-black text-[#94a3b8] uppercase">Grand Total</span><div className="flex items-center gap-2 text-[#0066b2] font-black"><CurrencySymbol className="text-[28px]" /><span className="text-[34px] tabular-nums">{grandTotal.toFixed(2)}</span></div></div>
              </div>
            </div>
      ...
      <Sheet open={isPartialSuccessOpen} onOpenChange={setIsPartialSuccessOpen}>
        <SheetContent side="bottom" className="rounded-t-[32px] border-none p-0 outline-none overflow-hidden h-[400px] flex flex-col tracking-normal">
          <SheetHeader className="sr-only"><SheetTitle>Payment In Progress</SheetTitle></SheetHeader>
          <div className="flex-1 flex flex-col items-center justify-center px-8 text-center pt-8"><div className="w-24 h-24 bg-[#00d084] rounded-full flex items-center justify-center shadow-[0_8px_24px_rgba(0,208,132,0.3)] mb-8"><Check className="w-12 h-12 text-white stroke-[5px]" /></div><h2 className="text-[24px] font-black text-[#1a1c2e] uppercase mb-4 leading-tight tracking-tight">PAYMENT IN PROGRESS</h2><p className="text-[#94a3b8] text-[16px] font-bold leading-tight mb-8">Some guests have paid. Please settle remaining shares.</p><Button onClick={() => setIsPartialSuccessOpen(false)} className="w-full h-16 bg-[#0066b2] hover:bg-[#005596] text-white rounded-[20px] text-[16px] font-black uppercase shadow-lg active:scale-95 transition-all">Continue</Button></div>
        </SheetContent>
      </Sheet>
    </div>
  );
